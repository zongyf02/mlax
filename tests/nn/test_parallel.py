...
def test_parallel(
    ...
    assert is_container(model.layers)
    assert isinstance(model.layers.states, list)

    assert_equal_pytree(t_acts, expected_train_output)
    assert is_container(new_t_model.layers)
    assert isinstance(new_t_model.layers.states, list)

    assert_equal_pytree(i_acts, expected_infer_output)
    assert is_container(new_i_model.layers)
    assert isinstance(new_i_model.layers.states, list)
